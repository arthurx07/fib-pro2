...
public:
  ...
  void removeFirstOccurrence(T value) {
    if (ptopitem != NULL) {
      if (ptopitem->value == value) pop();
      else {
        Item *pprev = ptopitem;
        bool found = false;
        while (not found and pprev->next != NULL) {
          if (pprev->next->value == value) {
            Item *paux = pprev->next;
            pprev->next = paux->next;
            delete paux;
            _size--;
            found = true;
          }
          if (not found) pprev = pprev->next;
        }
      }
    }
    /* Item *pitem = ptopitem;
