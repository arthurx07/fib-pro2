...
public:
  ...
  void moveTowardsEnd(iterator &it) {
    ...
    {
      // extractItem(it.pitem);
      // insertItem(it.pitem->next, it.pitem);
      // same as:
      it.pitem->prev->next = it.pitem->next;
      it.pitem->next->prev = it.pitem->prev;

      Item *pitemprev = it.pitem->next;
      it.pitem->prev = pitemprev;
      it.pitem->next = pitemprev->next;
      pitemprev->next->prev = it.pitem;
      pitemprev->next = it.pitem;
    }
  }
