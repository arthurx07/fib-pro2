  ...
  void swapFirstLast() {
    if (_size >= 2)
    {
      ...
      if (_size == 2)
      {
        ...
        itemsup.prev->prev = iteminf.next;
      }
      else
      {
        // we now have: 4 2 3 1, we are taking 3->next and assigning it to 1;
        iteminf.next->prev->next = itemsup.prev;
        // we now have: 4 2 3 1, we are taking 2->prev and assigning it to 4;
        itemsup.prev->next->prev = iteminf.next;

        iteminf.next->next = itemsup.prev->next;
        itemsup.prev->prev = iteminf.next->prev;
      }
