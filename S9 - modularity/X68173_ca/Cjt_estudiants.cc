...
void Cjt_estudiants::afegir_estudiant(const Estudiant& est) {
  ...
  if (imax == -1) imax = i+1;
  else if (vest[i+1].te_nota())
  {
    if (vest[imax].consultar_nota() < vest[i+1].consultar_nota())
      imax = i+1;
    else if (vest[imax].consultar_nota() == vest[i+1].consultar_nota() and vest[imax].consultar_DNI() > vest[i+1].consultar_DNI())
      imax = i+1;
  }
}
...
void Cjt_estudiants::modificar_estudiant(const Estudiant& est) {
  ...
  if (imax == -1) imax = i;
  else if (imax != i and vest[i].te_nota())
  {
    if (vest[imax].consultar_nota() < vest[i].consultar_nota())
      imax = i;
    else if (vest[imax].consultar_nota() == vest[i].consultar_nota() and vest[imax].consultar_DNI() > vest[i].consultar_DNI())
      imax = i;
  }
...
void Cjt_estudiants::modificar_iessim(int i, const Estudiant& est) {
  ...
  if (imax == -1) imax = i-1;
  else if (imax != i-1 and vest[i-1].te_nota())
  {
    if (vest[imax].consultar_nota() < vest[i-1].consultar_nota())
      imax = i-1;
    else if (vest[imax].consultar_nota() == vest[i-1].consultar_nota() and vest[imax].consultar_DNI() > vest[i-1].consultar_DNI())
  ...
  else if (imax == i-1) recalcular_posicio_imax();
}
...
void Cjt_estudiants::recalcular_posicio_imax()
{
  ...
  for (int i = 0; i < nest; ++i)
  {
    ...
    {
      if (vest[i].te_nota()
        and vest[imax].consultar_nota() < vest[i].consultar_nota())
        imax = i;
      else if (vest[i].te_nota()
              and vest[imax].consultar_nota() == vest[i].consultar_nota() and vest[imax].consultar_DNI() > vest[i].consultar_DNI())
        imax = i;
    }
    else if (vest[i].te_nota()) imax = i; // per trobar el primer est amb nota
  }
}
