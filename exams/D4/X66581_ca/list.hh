  ...
  class iterator {
       ...
       el resultat és el p.i. */
    {
      if (pitem != &(plist->itemsup)) {
        if (not hasActiveHook()) pitem = pitem->next;
        else if (hasActiveHook() and pitem->next != &(plist->itemsup)) {
          pitem->next->prev = pitem->prev;
          pitem->prev->next = pitem->next;

          Item* pitemprev = pitem->next;
          pitem->next = pitemprev->next;
          pitem->next->prev = pitem;
          pitem->prev = pitemprev;
          pitemprev->next = pitem;
       ...
       el resultat és el p.i. */
    {
      if (pitem != plist->iteminf.next) {
        if (not hasActiveHook()) pitem = pitem->prev;
        else {
          pitem->next->prev = pitem->prev;
          pitem->prev->next = pitem->next;

          Item* pitemprev = pitem->prev->prev;
          pitem->next = pitemprev->next;
          pitem->next->prev = pitem;
          pitem->prev = pitemprev;
          pitemprev->next = pitem;
